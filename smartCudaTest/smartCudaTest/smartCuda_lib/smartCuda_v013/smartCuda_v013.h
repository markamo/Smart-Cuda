...
template <typename T> 
__device__
	inline cudaError_t smartKernelFree(T* dev_mem)
	{
		cudaError_t cudaStatus;
		free(dev_mem);
		dev_mem = NULL;
...
template <typename T> 
__device__
	inline cudaError_t smartKernelFree(T* dev_mem, cudaError_t &cudaStatus)
	{
		////cudaError_t cudaStatus;
		free(dev_mem);
		dev_mem = NULL;
...
public:
	...
	__device__  __host__  smartIndex(int i,int j=1, int k=1, int w=1) 
	{
		if((i < 0 || j < 0 || k < 0) || (w < 0)) 
		{
			...
			fprintf(stderr, "Array Dimensions [%d][%d][%d][%d] of smart Array is out of bounds\n",i,j,k,w);			
			//exit(1);
		}
...
template <typename T> 
	inline __device__ T* smartKernelArray(int sizeX) 
{
	cudaError_t cudaStatus;
	T* dev_Array = 0;	
	int size = sizeX;
...
template <typename T> 
	inline __device__ T* smartKernelArray( int sizeX, int sizeY) ////, cudaError_t &cudaStatus)
{
	cudaError_t cudaStatus;
	T* dev_Array = 0;	
	int size = sizeX * sizeY;
...
template <typename T>
	inline __device__ T* smartKernelArray( int sizeX, int sizeY, int sizeZ) ////, cudaError_t &cudaStatus)
{
	cudaError_t cudaStatus;
	T* dev_Array = 0;	
	int size = sizeX * sizeY * sizeZ;
...
template <typename T>
	inline __device__ T* smartKernelArray( int sizeX, int sizeY, int sizeZ, int sizeW) ////, cudaError_t &cudaStatus)
{
	cudaError_t cudaStatus;
	T* dev_Array = 0;	
	int size = sizeX * sizeY * sizeZ * sizeW;
...
private: 
	...
	{	
		...
		//{
		//	// Take appropriate action here. This is just a placeholder response.
		//	fprintf(stderr, "Array not INITIALIZED or Index [%d][%d][%d][%d] of smart Array is out of bounds\n",i,j,k,w);			
		//	exit(1);
		//}
	...
	{		
		...
		//{
		//	// Take appropriate action here. This is just a placeholder response.
		//	fprintf(stderr, "View Index [%d][%d][%d][%d] of smart Array is out of bounds\n",i,j,k,w);			
		//	exit(1);
		//}
	...
	cudaError_t inner_copy(T* src_array, int src_type, int src_size, int src_device_id)
	{
		...
		else if (array_type == smartDevice || array_type == smartKernel) ////device -> host/device copies
		{
			if(src_type == smartHost || src_type == smartPinnedHost)
			{
				cudaStatus = smartCopyHost2Device<T>(dev_arr,src_array,copySize);
			}
			else if (src_type == smartDevice|| src_type == smartKernel)
			{
...
public: 
	...
	*/
	...
	T &operator()(int i) 
	{
		...
		{
			// Take appropriate action here. This is just a placeholder response.
			fprintf(stderr, "Array not INITIALIZED or Index [%d] of smart Array is out of bounds\n",i);			
			exit(1);
		}
		//T temp = dev_arr[i];
		idx = i;
	...
	T &operator()(int i, int j)
	{
		...
		{
			// Take appropriate action here. This is just a placeholder response.
			fprintf(stderr, "Array not INITIALIZED or Index [%d][%d] of smart Array is out of bounds\n",i,j);			
			exit(1);
		}
	...
	T &operator()(int i, int j, int k)
	{
		...
		{
			// Take appropriate action here. This is just a placeholder response.
			fprintf(stderr, "Array not INITIALIZED or Index [%d][%d][%d] of smart Array is out of bounds\n",i,j,k);			
			exit(1);
		}
	...
	cudaError_t &operator=(smartArrayWrapper<T,smartDevice> &other)
	{
		...
		if(array_type == smartHost || array_type == smartPinnedHost) ////host -> host/device copies
		{
			...
			else if (other.getType() == smartDevice || other.getType() == smartKernel)
			{
	...
	__device__  __host__
	cudaError_t &operator=(smartArrayWrapper<T,smartKernel> &other)
	{
		cudaError_t cudaStatus;
		//other._is_copy = true; ////prevent destructor from deleting array pointer
...
template <typename custom>
	...
	cudaError_t &operator=(custom other)
	{
		...
		if(array_type == smartHost || array_type == smartPinnedHost) ////host -> host/device copies
		{
			...
			else if (type == smartDevice || type == smartKernel)
			{
				if (device_id == dev_id)
				{
	...
	cudaError_t copy(smartArrayWrapper<T,smartPinnedHost> &other)
	{
		...
		__device__  __host__
	cudaError_t copy(smartArrayWrapper<T,smartKernel> &other)
	{
		cudaError_t cudaStatus;
		//other._is_copy = true; ////prevent destructor from deleting array pointer
	...
	cudaError_t copy(T* other, unsigned int other_size, int other_array_type = 1)
	{
		...
		if(array_type == smartHost || array_type == smartPinnedHost) ////host -> host/device copies
		{
			...
			else if (other_array_type == smartDevice || other_array_type == smartKernel)
			{
	...
	T viewAt_1D(int i)
	{
		...
		{
			// Take appropriate action here. This is just a placeholder response.
			fprintf(stderr, "View Index [%d] of smart Array is out of bounds\n",i);			
			exit(1);
		}
		vidx = 1;
		return dev_arr[i];
	...
	T viewAt_2D(int i, int j)
	{
		...
		{
			// Take appropriate action here. This is just a placeholder response.
			fprintf(stderr, "View Index [%d][%d] of smart Array is out of bounds\n",i,j);			
			exit(1);
		}
		int _idx = i + j * vX;
		vidx = _idx;
	...
	T viewAt_3D(int i, int j, int k)
	{
		...
		{
			// Take appropriate action here. This is just a placeholder response.
			fprintf(stderr, "View Index [%d][%d][%d] of smart Array is out of bounds\n",i,j,k);			
			exit(1);
		}
...
template <typename T> 
	inline __device__  __host__
		cudaError_t smartFree(smartArrayWrapper<T,smartKernel> other)
	{
		cudaError_t cudaStatus;
		int array_type = other.getType();
		T* dev_mem = other.inner_ptr();
		...
		else if (array_type == smartKernel)
		{
			free(dev_mem);
			cudaStatus = cudaGetLastError();
			if (cudaStatus != cudaSuccess) {
...
public:
	...
	cudaError_t  setDeviceLimit(int memory_size)
	{
		...
		if (cudaStatus != cudaSuccess) {
			fprintf(stderr, "cudaDeviceSetLimit returned error code %d!\n", cudaStatus);
